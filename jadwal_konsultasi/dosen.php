    ...
    if (empty($nip) || empty($nama_dosen) || empty($kategori)) {
        ...
        if (mysqli_num_rows($cek) > 0) {
            ...
            if ($pesan_error == "") {
                ...
                if (!$insert)
                    $pesan_error = "Gagal menambahkan dosen.";
            }
        }
    }
}
                        ...
                        <form method="POST" enctype="multipart/form-data" id="formTambah">
                            ...
                            <div class="mb-3">
                                <label class="fw-semibold small text-muted text-uppercase mb-1">
                                    Foto Profil <span class="fw-normal text-muted">(Opsional)</span>
                                </label>
                                <input type="file" id="inputFoto" name="foto" class="form-control form-control-file-sm" accept="image/png,image/jpeg,image/jpg">
                                <div id="statusFoto">
                ...
                <div class="card main-card">
                    ...
                    <div class="card-header card-header-modern border-bottom">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                            <span>
                                <i class="bi bi-grid-3x3-gap-fill text-success me-2"></i> Daftar Dosen
                            </span>
                            <form method="GET" class="search-box">
                                <i class="bi bi-search"></i>
                                <input type="text" name="cari" class="form-control" style="min-height:44px; font-size:14px;" placeholder="Cari NIP, Nama atau Keahlian..." value="<?= htmlspecialchars($cari) ?>">
                            </form>
                        </div>
                    </div>
                            ...
                            if (mysqli_num_rows($query_dosen) > 0):
                                while ($row = mysqli_fetch_assoc($query_dosen)):
                                    $foto_src = (!empty($row['foto']) && $row['foto'] != 'default.png')
                                        ? 'uploads/' . $row['foto']
                                        : 'https://ui-avatars.com/api/?name=' . urlencode($row['nama_dosen']) . '&background=10b981&color=fff&size=200&rounded=true';
                                    ?>

                                    <div class="col-sm-6">
                                        <div class="dosen-card h-100 p-4 text-center">

                                            <img src="<?= $foto_src ?>" class="foto-profil mb-3" alt="Foto <?= htmlspecialchars($row['nama_dosen']) ?>" onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($row['nama_dosen']) ?>&background=10b981&color=fff&size=200'">

                                            <h6 class="fw-bold text-primary mb-1"><?= htmlspecialchars($row['nama_dosen']) ?></h6>
                                            <p class="text-muted small mb-2"><?= htmlspecialchars($row['nip']) ?></p>
                                            <span class="badge-keahlian mb-3"><?= htmlspecialchars($row['kategori_keahlian']) ?></span>

                                            <div class="d-flex gap-2 justify-content-center mt-2">
                                                <button class="btn btn-outline-warning btn-sm fw-semibold px-3" style="border-radius:10px;" data-bs-toggle="modal" data-bs-target="#editModal<?= $row['id_dosen'] ?>">
                                                    <i class="bi bi-pencil-square me-1"></i>Edit
                                                </button>
                                                <a href="?hapus=<?= $row['id_dosen'] ?>" class="btn btn-outline-danger btn-sm btn-hapus fw-semibold px-3" style="border-radius:10px;">
                                                ...
                                                <form method="POST" id="formEdit<?= $row['id_dosen'] ?>">
                                                    ...
                                                    <div class="modal-header border-0 pb-0">
                                                        <h5 class="modal-title fw-bold">
                                                            <i class="bi bi-pencil-square text-success me-2"></i> Edit Dosen
                                                        </h5>
                                                    ...
                                                    <div class="modal-body px-4">
                                                        ...
                                                        <div class="text-center mb-4">
                                                            <div class="position-relative d-inline-block">
                                                                <img src="<?= $foto_src ?>" id="fotoPreview<?= $row['id_dosen'] ?>" style="width:100px;height:100px;border-radius:50%;object-fit:cover;border:4px solid #fef3c7;box-shadow:0 4px 15px rgba(245,158,11,.25);" alt="Foto Profil" onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($row['nama_dosen']) ?>&background=10b981&color=fff&size=200'">
                                                                <label for="inputFotoProfil<?= $row['id_dosen'] ?>" style="position:absolute;bottom:0;right:0;width:32px;height:32px;background:linear-gradient(135deg,#f59e0b,#ef4444);border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid white;cursor:pointer;" title="Ganti Foto">
                                                                    <i class="bi bi-camera-fill" style="font-size:12px;color:white;"></i>
                                                                </label>
                                                            </div>
                                                            <input type="file" id="inputFotoProfil<?= $row['id_dosen'] ?>" class="d-none inputFotoEdit" data-id="<?= $row['id_dosen'] ?>" accept="image/png,image/jpeg,image/jpg">
                                                            <input type="hidden" name="cropped_foto_edit" id="croppedFotoEditInput<?= $row['id_dosen'] ?>">
                                                            
                                                            <div id="statusFotoEdit<?= $row['id_dosen'] ?>" class="mt-2">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="text" name="kategori_keahlian" class="form-control" value="<?= htmlspecialchars($row['kategori_keahlian']) ?>" required>
                                                        </div>
                                                        <div class="mb-3">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 rounded-4 overflow-hidden">
                <div class="modal-header text-white border-0" style="background:var(--grad)">
                    <h5 class="modal-title fw-bold"><i class="bi bi-crop me-2"></i>Sesuaikan Foto Profil</h5>
